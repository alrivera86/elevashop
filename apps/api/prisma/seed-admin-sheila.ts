...
async function main() {
  ...
  const existingUser = await prisma.usuario.findUnique({
    where: { email: 'shbriceno@elevashop.com' }
  });

  if (existingUser) {
    console.log('\nEl usuario shbriceno@elevashop.com ya existe (ID:', existingUser.id + ')');
  ...
  // Crear usuario
  const password = 'Elevashop2024!'; // Contraseña temporal
  const hashedPassword = await bcrypt.hash(password, 10);

  const usuario = await prisma.usuario.create({
    data: {
      email: 'shbriceno@elevashop.com',
